function paramChanges=MAPparamsNormalMar25 ...
    (BFlist, sampleRate, showParams, paramChanges, showParamsFileName)
...
%    BFlist= -1, implies that the defaults should be used
%    BFlist= [lowest highest N] specifies N BFs equally spaced on a 
%    logarithmic scale between 'lowest' and 'highest'.
...
%
% (just print out this file with no parameter changes)
%   MAPparamsNormal(-1, 48000, 1)
...
if nargin<5,showParamsFileName='';end
if nargin<4,paramChanges={};end
if nargin<3, showParams=0; end
if nargin<2, sampleRate=48000; end
if nargin<1 || BFlist(1)<0 || length(BFlist)==3
    % set BFlist to default
...
% this file should normally be empty but worth checking.
...
DRNLParams.a=ones(length(BFlist),1)* 2e3;  
DRNLParams.c=.25;        % compression exponent
DRNLParams.ctBMdB = 15;  % compression knee (dB re referenceDisplacement)
...
DRNLParams.nlBWp=0.14; DRNLParams.nlBWq=180;
...
DRNLParams.rateToAttenuationFactor= 2e3*[1 .05 0 ];
DRNLParams.MOCtau = [.08 .4 1];
DRNLParams.MOCTotThreshold=0;            % IC sp/s, NB spont rate= 0

% 'probability' model: MOC based on AN probability (HSR)
DRNLParams.MOCrateThresholdProb =60;                % spikes/s 
DRNLParams.rateToAttenuationFactorProb = 2e3*[1 .05 0 ];
DRNLParams.MOCtauProb = [.08 .4 1];
...
% IHC_cilia_RPParams.C=	.4;          % BM disp/ cilia disp scalar 
...
IHC_cilia_RPParams.Gmax= 6e-9;  % 2.5e-9 maximum conductance (Siemens)
IHC_cilia_RPParams.Ga= .8e-9;  % 4.3e-9 'always on' apical conductance
...
switch MacGregorMultiType
    ...
    case 'chopper'
        ...
        MacGregorMultiParams.currentPerSpike=17e-9; % *per spike       
...
MacGregorParams.currentPerSpike=56e-9;% *(A) per spike
