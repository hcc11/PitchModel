...
if length(BFlist)>3
    ...
    OMEParams.rateToAttenuationFactor=0;        % uses all level 2 spikes
    OMEParams.ARrateThreshold=0; % spikes/s IC (LSR) neurons
    % 'probability model': AR based on AN firing probabilities (LSR)
    OMEParams.rateToAttenuationFactorProb=0;    % uses all AN rates
...
DRNLParams.a=ones(length(BFlist),1)* 1.2e6*0.0025; % correction for different references
DRNLParams.c=.25;        % compression exponent
DRNLParams.ctBMdB = 0;  % compression knee (dB re referenceDisplacement)
...
DRNLParams.rateToAttenuationFactor= 3e3*[.9 .1 0 ];
...
DRNLParams.rateToAttenuationFactorProb = (395/182)*3e3*[.9 .1 0];
DRNLParams.MOCtauProb = [.055 .4 1];
...
switch IHC_cilia_RPParams.IHCmodel
    case 'Shamma'
        % Shamma
        IHC_cilia_RPParams.tc=	0.0003;    % 0.0003 Shamma
        IHC_cilia_RPParams.C=	.1;          % BM disp/ cilia disp scalar
        
        % Boltzman function parameters
        IHC_cilia_RPParams.u0=	1e-8;
        IHC_cilia_RPParams.s0=	3e-8;
        IHC_cilia_RPParams.u1=	1e-8;
        IHC_cilia_RPParams.s1=	9e-8;
      
        IHC_cilia_RPParams.Gmax= 7.5e-9;  % 2.5e-9 maximum conductance (Siemens)
        IHC_cilia_RPParams.Ga= 0.25e-9;  % 4.3e-9 'always on' apical conductance
                
        %  IHC_RP
        IHC_cilia_RPParams.Cab=	16e-012;    % IHC capacitance (F/cm^2)
        %         IHC_cilia_RPParams.Cab=	1e-012;    % IHC capacitance (F/cm^2)
        IHC_cilia_RPParams.Et=	0.100;     % endocochlear potential (V)
        
        IHC_cilia_RPParams.Gk=	1.8e-008;  % 1e-8 potassium conductance (S)
        IHC_cilia_RPParams.Ek=	-0.0705;     % -0.084 K equilibrium potential (V)
%         IHC_cilia_RPParams.Ek=	-0.08;     % -0.084 K equilibrium potential (V)
        IHC_cilia_RPParams.Rpc=	0.04;      % combined resistances (ohms/cm^2)
        
                % Boltzman function parameters (Normal)
...
%%  #5 IHCpreSynapse
IHCpreSynapseParams=[];
IHCpreSynapseParams.GmaxCa=	8e-9;% maximum calcium conductance
...
IHCpreSynapseParams.tauM=	0.0001;% membrane time constant ?0.1ms
...
IHCpreSynapseParams.tauCa=  [.000075]; %LSR, MSR and HSR fiber
...
AN_IHCsynapseParams.y=	3;   % depleted vesicle replacement rate
AN_IHCsynapseParams.x=	30; % replenishment from re-uptake store
AN_IHCsynapseParams.l=	2580;  % *loss rate of vesicles from the cleft
AN_IHCsynapseParams.r=	6580; % *reuptake rate from cleft into cell
...
switch MacGregorMultiType
    ...
    case 'chopper'
        ...
        MacGregorMultiParams.currentPerSpike=4e-8; % *per spike
        ...
        MacGregorMultiParams.tauGk=	0.001;% K+ conductance tau (s)
        MacGregorMultiParams.Th0=	0.01; % equilibrium threshold (V)
...
MacGregorParams.currentPerSpike=3e-7;% *(A) per spike
...
MacGregorParams.tauGk=	0.001;     % K+ conductance tau (s)
