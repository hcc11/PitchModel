...
function [psy, levelsPhaseTwoBinVector, logistic, rareEvent]= ...
    bestFitPsychometicFunctions (levelsPhaseTwo, responsesPhaseTwo)
% bestFitPsychometicFunctions computes a psychometric function from 
% a matrix of levels and responses output
%  psy is the empirical probabbility of a detection associated with 
%   levels in levelsPhaseTwoBinVector
%  logistic is a structure with results for fitting the logistic function
%    the logistic fit depends on whether or not maximum likelihood is used 
...
[psy, levelsPhaseTwoBinVector, binFrequencies,yesResponses, noResponses]=...
    psychometricFunction(levelsPhaseTwo, responsesPhaseTwo, ...
...
if isempty(psy)
    ...
    logistic.bestThreshold=NaN; logistic.bestK=NaN;  
    logistic.predictionsLOG=[]; logistic.predictionLevels=[];
    rareEvent.bestGain=NaN; 
    rareEvent.bestVMin=NaN; 
    rareEvent.predictionsRE=[]; 
    rareEvent.predictionLevels=[];
    rareEvent.thresholddB= NaN; 
...
switch experiment.paradigm
    case 'gapDuration'
        % don't attempt this but visit the function to set null results
        rareEvent=fitRareEvent([], responsesPhaseTwo, ...
            stimulusParameters.targetDuration);
    otherwise
        rareEvent=fitRareEvent(levelsPhaseTwo, responsesPhaseTwo, ...
            stimulusParameters.targetDuration);
end
...
switch experiment.functionEstMethod
    ...
    case {'logisticLS', 'rareEvent','peaksAndTroughs'}
        [a1, a2, Euclid]= ...
            fitFunctionUsingLeastSquares(levelsPhaseTwo, responsesPhaseTwo, ...
            logisticFunc, experiment.possLogSlopes, experiment.meanSearchStep);
        logistic.bestThreshold=a1;
        logistic.bestK=a2;
        logistic.predictionsLOG= 1./(1+exp(-logistic.bestK.*...
            (experiment.predictionLevels-logistic.bestThreshold)));
    ...
    case 'logisticML'
        [a1, a2, Euclid]= ...
            fitFunctionUsingMaxLikelihood (levelsPhaseTwoBinVector,...
            psy, yesResponses, noResponses, logisticFunc, ...
            experiment.possLogSlopes,experiment.meanSearchStep);
        logistic.bestThreshold=a1;
        logistic.bestK=a2;
        logistic.predictionsLOG= ...
            1./(1+exp(-logistic.bestK.*...
            (experiment.predictionLevels-logistic.bestThreshold)));
        logistic.predictionLevels=experiment.predictionLevels;
        logistic.Euclid = Euclid;
end
...
function [a1, a2, Euclid]=fitFunctionUsingLeastSquares(levelsPhaseTwo, ...
    responsesPhaseTwo, func, possSlopes, meanSearchStep)
...
function [a1, a2, Euclid]=fitFunctionUsingMaxLikelihood...
    (levelsPhaseTwo,psy, yesResponses, noResponses, func, ...
    possible_a2, meanSearchStep)

% fitFunctionUsingMaxLikelihood fits the function in 'func' to 
%   binned yes-no data.
% levelsPhaseTwo specifies the bin centers
% yesResponses and noResponses are the niumber of yes and no 
%  responsesPhaseTwo in each bin
...
% --------------------------------------------------- fitRareEvent
function rareEvent=fitRareEvent(stimulusLevels, responsesPhaseTwo, ...
    duration, gains, Vmins)
...
%             predictions(~idx)=0;
            ...
            % NB the error is equally weighted for each 
            %  stimulus/response pair. This is not the same as 
            %  equal weighting for each distinct level
...
rareEvent.thresholdPa=(-log(0.5)/duration + ...
    rareEvent.bestVMin)/rareEvent.bestGain;
