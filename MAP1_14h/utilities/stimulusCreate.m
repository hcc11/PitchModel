function [audio, errorMessage, time]=stimulusCreate(globalStimParams,...
    stimComponents)
% updated August 2012
%
% stimulusCreate is a flexible utility for generating stereo stimuli
%
%  stimComponents:
% The charactersitics are described in the structure 'stimComponents'.
% The stimulus may be complex in which case it is described in terms of a
%  number of simple components each containing a single burst of activity.
% These components are then added together before the function returns it.
% If the stimulus contains more than one component, stimComponents is
%  a structure array. 
% If the components are different in left and right
%  ears then it becomes a two-dimensional array.
%
%  globalStimParams:
% The sample rate applies to all components and is specified in the
%  structure globalStimParams as the field FS.
% The overall duration of the stimulus is specified in the field
% 'overallDuration'. This is needed to force all components to be the same
%  overall length before addition or to determine that they cannot be
%  accommodated (producing an error). Each individual burst of activity
%  (i.e. tone or noise) has a duration and an associated preceding silence
%  duration. The terminal silence duration should be set to -1 to allow the
%  program to add a suitable duration to equalise all the components.
% (** endSilence is strictly redundant and should not be in here at all)
%
%  Optional fields:
%  globalStimParams.filter = [leftfl leftfu rightfl right fu]
%    filter is applied separately to left and right combined sounds
%    NB individual components can also be filtered (see below)
%
%  globalStimParams.ears=x; % where:
%  x='monoticL', 'monoticR', 'diotic', or 'dichotic'
%
%  A (calibration) correction factor is applied to all signals to 
%   compensate for differences in output devices.
% globalStimParams.audioOutCorrection=2;
%  where audioOutCorrection is a scalar
%
%  globalStimParams.FFT= 1; % {0, 1} (default=0)
%  globalStimParams.doPlot=1; % {0, 1} plots stimulus (default=0)
%  globalStimParams.doPlay=1; % {0, 1} generates audio output (default=0)
%
% Output arguments:
...
%  globalStimParams.overallDuration=.1; 
%
...
% stimComponents.frequencies=500;
% stimComponents.amplitudesdB=50;
% stimComponents.beginSilence=.01;
% stimComponents.endSilence=-1;
% stimComponents.rampOnDur=.002;
% stimComponents.rampOffDur=-1;
% stimComponents(ear, componentNo).filter=[100 10000 2] 
% NB [lower, upper, order] applies to an individual component
%
% [audio, errorMessage]=stimulusCreate(globalStimParams, stimComponents);
%
% Multi-component stimuli
%  mandatory structure fields:
%  globalStimParams.FS=100000;
%  globalStimParams.overallDuration=.1;  % s
%
% (ear=1 or 2;  componentNo= any number)
...
% stimComponents(ear, componentNo).frequencies=1000;
% stimComponents(ear, componentNo).amplitudesdB=50;
% stimComponents(ear, componentNo).beginSilence=0;
% stimComponents(ear, componentNo).endSilence=0;
% stimComponents(ear, componentNo).rampOnDur=.005;
% stimComponents(ear, componentNo).rampOffDur=0.005;
% stimComponents(ear, componentNo).filter=[100 10000 2] 
% (These values are defaults if a field is missing).
%
% [audio, errorMessage]=stimulusCreate(globalStimParams, stimComponents);
%
% stimComponents.type='noise';    % {'IRN', 'irn', 'noise', 'pinkNoise'}
% stimComponents.toneDuration=.05;;
% stimComponents.amplitudesdB=50;
% stimComponents.beginSilence=.01;
% stimComponents.endSilence=-1;
% stimComponents.rampOnDur=.002;
% stimComponents.rampOffDur=-1;
% [audio, errorMessage]=stimulusCreate(globalStimParams, stim);
%
% % for IRN only
% stimComponents.type='IRN';
% stimComponents.toneDuration=.05;;
% stimComponents.amplitudesdB=50;
% stimComponents.beginSilence=.01;
% stimComponents.endSilence=-1;
% stimComponents.rampOnDur=.002;
% stimComponents.rampOffDur=-1;
% stimComponents.niterations = 8;   %0 for white noise
% stimComponents.delay = 1/150;
% stimComponents.irnGain = 1;
% [audio, errorMessage]=stimulusCreate(globalStimParams, stim);
%
% % for notched noise only
% stimComponents.type='notchedNoise';
% stimComponents(ear, componentNo).frequencies=1500; % centre of notch
% stimComponents.toneDuration=.05;;
% stimComponents.amplitudesdB=50;
% stimComponents.beginSilence=.01;
% stimComponents.endSilence=-1;
% stimComponents.rampOnDur=.002;
% stimComponents.rampOffDur=-1;
% stimComponents.notchedNoiseBW = 100;  % Hz
% stimComponents.bandWidth = 800;   % Total bandwidth of the noise
% [audio, errorMessage]=stimulusCreate(globalStimParams, stim);
%
% stimComponents.type='clicks';    % uses frequencies for duty cycle
% stimComponents.toneDuration=.05;;
% stimComponents.amplitudesdB=50;
% stimComponents.beginSilence=.01;
% stimComponents.endSilence=-1;
...
% stimComponents.clickTimes=[.4 .6];  
% times in cycle when clicks occur, default = 1
%

errorMessage=''; %error messages; no message is a good message

if nargin<2, stimComponents=[]; end     % will generate defaults
if nargin<1, globalStimParams=[]; end   % will generate defaults
if ~isfield(globalStimParams,'doPlot') || isempty (globalStimParams.doPlot)
    globalStimParams.doPlot=0;
end

if ~isfield(globalStimParams,'FS') || isempty (globalStimParams.FS)
    globalStimParams.FS=50000;
end

if ~isfield(globalStimParams,'overallDuration') || isempty (globalStimParams.overallDuration)
    globalStimParams.overallDuration=0.05;
end

if length(stimComponents)<2
    % ** could be made to apply to all stimComponents
    % Only one stimulus component; fill in the defaults
    if ~isfield(stimComponents,'type') || isempty (stimComponents.type)
        stimComponents.type='tone';
    end
    if ~isfield(stimComponents,'phases') || isempty (stimComponents.phases)
        stimComponents.phases='sin';
    end
    if ~isfield(stimComponents,'toneDuration') || isempty (stimComponents.toneDuration)
        stimComponents.toneDuration=0.05;
    end
    if ~isfield(stimComponents,'frequencies') || isempty (stimComponents.frequencies)
        stimComponents.frequencies=1000;
    end
    if ~isfield(stimComponents,'amplitudesdB') || isempty (stimComponents.amplitudesdB)
        stimComponents.amplitudesdB=50;
    end
    if ~isfield(stimComponents,'beginSilence') || isempty (stimComponents.beginSilence)
        stimComponents.beginSilence=0;
    end
    if ~isfield(stimComponents,'endSilence') || isempty (stimComponents.endSilence)
        stimComponents.endSilence= 0;
    end
    if ~isfield(stimComponents,'rampOnDur') || isempty (stimComponents.rampOnDur)
        stimComponents.rampOffDur=0.005;
    end
    if ~isfield(stimComponents,'notchedNoiseBW') || isempty (stimComponents.notchedNoiseBW)
        stimComponents.notchedNoiseBW=100;
    end
    if ~isfield(stimComponents,'bandWidth') || isempty (stimComponents.bandWidth)
        stimComponents.bandWidth=0.005;
    end
end
% special single component case;
%  allow user to omit overallDuration or get it wrong
if length(stimComponents)==1 && ~(stimComponents.endSilence<0)
    globalStimParams.overallDuration=stimComponents.toneDuration+...
        stimComponents.beginSilence+ stimComponents.endSilence;
end

% perform checks and set defaults
% add nSignalPoints & dt to globalStimParams
...
% create each stimulus component individually
[Nears, nComponentSounds]=size(stimComponents);
for ear=1:Nears % left/ right
    % combinedSignal is the sum of all sounds in one ear (a column vector)
    ...
    for componentNo=validStimComponents
        ...
        switch stim.type
            ...
            case { 'notchedNoise'}
                stimulus=makeNotchedNoise(globalStimParams, stim);
            ...
            case 'singleGauss'
                stimulus = 20e-6*make_singlegauss(globalStimParams, stim);
                stim.rampOnDur=NaN;
                
            case 'multipleGauss'
                SR= globalStimParams.FS;
                
                Ncomp = 4;
%                 fms   = [4000 ; 4000 ; 4000 ; 4000];
                fms   = repmat(stim.frequencies,1,4);
                dTs   = [-24 ; -16 ; -8 ; +8]; %(3 forward & 1 backward maskers)
%                 Lms   = [83.7 ; 76.3 ; 66.3 ; 80.7]; %(mean values across listeners)
                Lms   = stim.amplitudesdB; 
                ft    = stim.frequencies;
                Lt    = -100;
                
                stimulus = 20e-6* make_multiplegauss(SR,Ncomp,fms,dTs,Lms,ft,Lt);
                stim.rampOnDur=NaN;
                ...
                switch stim.type(end-5:end)
                    ...
                    case 'Babble'
                        ...
                        if ~isequal(sampleRate, globalStimParams.FS)
                            ...
                            errorMessage= ['error: file sample rate disagrees ' ...
        ...
        if endOfStimulus< 0
            error('stimulusCreate: negative tone duration found')
        end
...
if globalStimParams.doPlot
    ...
    set(gcf,'name','stimulusCreate stimulus')
    if Nears>1
...
if globalStimParams.doPlay
    if ispc
        audioplayer(audio,globalStimParams.FS)
    else
...
for i=1:length(stim.transModFreq)
    ...
    [envelope, errorMessage]=stimulusCreate(globalStimParams, stim);    % NB recursive
...
if max(checkClickTimes)>1
    errorMessage= 'click times must be <= than 1 (period)';
    return
end

if clickWidth>=1/stimComponents.clickRepeatFrequency
    errorMessage= 'click width is too great for frequency';
    return
end
...
noise=UTIL_Butterworth (noise,  1/FS, 100, 10000, 6);
rms=(mean(noise.^2)^.5);  %should be 20 microPascals for 0 dB SPL
adjust=20e-6/rms;
noise=noise*adjust;
rms=(mean(noise.^2)^.5);
...
% bandpassed white noise with a notch centered on 'frequencies'.
% noise band is also centered on 'freqeuncies'
%
% % You need
%
% stimComponents.type='notchedNoise';
% stimComponents.frequencies=1500;  % centre of notch
% stimComponents.toneDuration=.05;  % noise duration
% stimComponents.amplitudesdB=50;
% stimComponents.beginSilence=.01;
% stimComponents.endSilence=-1;
...
% stimComponents.rampOffDur=-1;
% stimComponents.notchedNoiseBW = 100;  % Hz
% stimComponents.maskerNoiseBW  = 800;  % Total noise bandwidth
% stimComponents.maskerNoiseBW  = -1;  % 20-10 kHz
% 
% % Mandatory structure fields
% globalStimParams.FS=100000;
...
%              Spectrum level (dB SPL/Hz) + 10*log10(Noise bandwidth)
% e.g. for 10kHz bandwidth add 40 dB


notchCenter=stimComponents.frequencies;
notchedNoiseBW=stimComponents.notchedNoiseBW;
lowerNotchEdge=round(notchCenter-notchedNoiseBW/2);
upperNotchEdge=round(notchCenter+notchedNoiseBW/2);

bandWidth=stimComponents.maskerNoiseBW;
lowerNoiseEdge=round(notchCenter-bandWidth/2);
upperNoiseEdge=round(notchCenter+bandWidth/2);
lowerNoiseEdge=20;
upperNoiseEdge=10000;

noise=UTIL_notchedNoise ...
    (lowerNoiseEdge, upperNoiseEdge, lowerNotchEdge,upperNotchEdge,...
    stimComponents.toneDuration,globalStimParams.FS, ...
    stimComponents.amplitudesdB);
...
% ------------------------------------------- singleGaussian
function g = make_singlegauss(globalStimParams, stim)
...
% function g = make_multiplegauss(globalStimParams, stim)
...
%-------------------------------------------------------------------- descriptorError
function descriptorError( errorMessage, stimComponents, ear, componentNo)
