...
% updated August 2012
% stimulusCreate is a flexible utility for generating stimuli whose
%  charactersitics are described in the structure 'stimComponents'.
...
% If the stimulus contains more than one component, then stimComponents is
%  a structure array. If the components are different in left and right 
...
% (NB, endsilence is strictly redundant and should not be in here at all)
% 
%
...
% [audio, errorMessage]=stimulusCreate(globalStimParams, stim, doPlot);
%
...
% These are defaults if no arguments are presented or come are missing.
% 
...
% [audio, errorMessage]=stimulusCreate(globalStimParams, stim);stimComponents.clickWidth;
%
% stimComponents.type='clicks';    % uses frequencies for duty cycle
% stimComponents.toneDuration=.05;;
% stimComponents.if ~isfield(stimComponents.'toneDuration') || isempty (stimComponents.toneDuration)
...
% stimComponents.clickTimes=[.4 .6];            % times in cylce when clicks occur, default = 1
%
%

errorMessage=''; %error messages; no message is a good message

if nargin<2, stimComponents=[]; end
if nargin<1, globalStimParams=[]; end
...
if length(stimComponents)<2
    ...
    if ~isfield(stimComponents,'amplitudesdB') || isempty (stimComponents.amplitudesdB)
        stimComponents.amplitudesdB=1000;
    end
    ...
    if ~isfield(stimComponents,'rampOnDur') || isempty (stimComponents.rampOnDur)
        stimComponents.rampOnDur=0.005;
    end
end
                                        ...
                                        function noise=makeNoise(globalStimParams, stimComponents)
                                            ...
                                            function noise=makeWhiteNoise(globalStimParams, stimComponents)
                                                ...
                                                function noise=makeNotchedNoise(globalStimParams, stimComponents)
                                                    ...
                                                    % This code is white noise but should be bandstopped noise!!
