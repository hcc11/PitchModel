function vectorStrength=testAN_DG(probeFrequency, channelBF, levels, ...
    paramsName,paramChanges, getTimeconstants)
% testAN_DG generates rate/level functions for AN and brainstem units.
...
spikesSampleRate=24000; % multiple of 250->8000 Hz (incl 6 kHz)
...
onsetPTR=round(silenceDuration*spikesSampleRate);
offsetPTR=round((silenceDuration+toneDuration)*spikesSampleRate);
...
paramChanges{length(paramChanges)+1}=...
['AN_IHCsynapseParams.numFibers='	num2str(500)];
% signal ramp
...
for leveldB=levels
    ...
    figure(21), subplot(ceil(length(levels)/2),2,levelNo),cla, pause(0.1)
    
%         if exist('experiment') && isfield(experiment, 'stop') && experiment.stop
%             return
%         end
    ...
    if nFiberTypes< 2
        error('testAN_DG: both LSR and HSR fiber types must be specified')
    end
    ...
    LSRspikes=ANoutput(1:numLSRfibers,onsetPTR:offsetPTR);
    ...
    % AN HSR response
    HSRspikes= ANoutput(end- numHSRfibers+1:end, onsetPTR:offsetPTR);
    ...
    PSTH_LSR=UTIL_PSTHmaker(LSRspikes, dtSpikes, localPSTHbinwidth);
    PSTH_LSR=mean(PSTH_LSR,1)/localPSTHbinwidth;  % across fibers rates
    hold on,  bar(PSTHtime,PSTH_LSR,'w')
    ylim([0 1000])
    xlim([0 length(PSTH_HSR)*localPSTHbinwidth])
    grid on
    set(gcf,'name',['PSTH_HSR: ' num2str(channelBF), ' Hz: ' num2str(leveldB) ' dB']);

        figure(21), subplot(ceil(length(levels)/2),2,levelNo)
    hold off, plot(PSTHtime,PSTH_HSR, 'b')    
    hold on,  plot(PSTHtime,PSTH_LSR,'r')
    title(num2str(leveldB))
    ylim([0 1000])
    xlim([0 length(PSTH_HSR)*localPSTHbinwidth])
    grid on
    figure(5), subplot(2,2,2)
    ...
    CNLSRspikes=CNoutput(1:nLSRneurons, onsetPTR:offsetPTR);
    ...
    MacGregorMultiHSRspikes= CNoutput(end-nLSRneurons+1:end, onsetPTR:offsetPTR);
    ...
    ICLSRspikes=ICoutput(1:nLSRneurons, onsetPTR:offsetPTR);
