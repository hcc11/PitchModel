...
paramChanges={'IHCpreSynapseParams.tauCa=  [50e-6 190e-6];'};    % no changes

% Parameter changes can be used to change one or more model parameters
%  *after* the MAPparams file has been read
%  Each string must have the same format as the corresponding line in the
%   file identified in 'MAPparamsName'
% This example declares two fiber types with IHC calcium clearance time
% constant of 50e-6 and 190e-6 s (LSR and HSR fibers).
...
showMapOptions.printFiringRates=1;      % summarise output (command window)
...
% figure(99),set(gcf,'units','normalized','position',[0 0 0.33,1])
% figure(97),set(gcf,'units','normalized','position',[0.33 0 0.33,1])
% figure(98),set(gcf,'units','normalized','position',[0.66 0 0.33,1])
toc
path(restorePath);
